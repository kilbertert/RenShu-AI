...
一个基于 FastAPI + react + LangGraph 构建的前后端分离多智能体中医问诊系统，支持 DeepSeek（中医微调版）、TCM-LLM 等中医领域大模型，融合 Agent 动态推理与 GraphRAG 中医知识检索能力，解决中医问诊场景下**意图识别准确率低**、**结构化 / 非结构化数据割裂**、**辨证推理不严谨**等核心痛点，覆盖中医辨证、古籍查询、医案参考、药材咨询、养生指导等全场景需求。
...
2. **中医数据割裂严重**：结构化数据 (患者病例) 与非结构化数据（《黄帝内经》《伤寒论》等古籍、临床医案、舌苔图片）缺乏关联，无法支撑 "辨证 - 查典 - 荐方" 的闭环；
...
2. 搭建中医**混合知识引擎**，打通结构化（患者信息）与非结构化（古籍、医案）数据，支撑辨证推理与知识检索；
...
| ---------- | ---------- | ----------------------------------------------------------- | ----------------------------------------------------------- |
| 结构化数据 | PostgreSQL      | 患者表（id / 姓名 / 体质）、病例表（id / 症状 / 辨证结果）、  |
...
- 数据存储：PostgreSQL（结构化数据：患者、病例）、Neo4j（图数据：药材 - 证型 - 方剂关系）
...
- 核心框架：react（组件化开发）、javaScript（类型安全）
- UI 组件库：ant desgin（适配中医场景的主题定制）
- 特色组件：
